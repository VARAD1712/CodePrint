...
> **Note on Windows and Script Paths:**
> On Windows, use `python` instead of `python3`. When executing the search script, always use either the workspace path `.agents/skills/ui-ux-pro-max/scripts/search.py` or the global customization path `C:\Users\Raj Chaudhari\.gemini\config\skills\ui-ux-pro-max\scripts\search.py` (e.g., `python "C:\Users\Raj Chaudhari\.gemini\config\skills\ui-ux-pro-max\scripts\search.py" "<query>" --design-system`).
